...
def update_process_status(cursor, connection, process_name):
    ...
    cursor.execute("INSERT INTO process_python2 (id, current_process) VALUES (%s, %s)", (str(uuid.uuid4()), process_name))
    connection.commit()
...
def insert_lithium_price(cursor, connection, lithium_data):
    """Insert lithium price data into the database (reusing api_app_cmecopperspot table)"""
    try:
        # First, delete any existing records from today
        delete_query = """
        DELETE FROM api_app_cmecopperspot 
        ...
        # SQL query to insert data into the table (reusing copper table structure)
        # We'll use globex_code field to store 'LITHIUM' identifier
        insert_query = """
        INSERT INTO api_app_cmecopperspot (
            id, globex_code, last_price, price_change, price_change_percent, 
            volume, is_decrease, source, scraped_at, created_at, updated_at
        ...
        # Data to be inserted - mapping lithium data to existing table structure
        lithium_price_data = (
            str(uuid.uuid4()),  # id (UUID)
            'LITHIUM_CNY',  # globex_code (identifier)
            lithium_data['price'],  # last_price
            lithium_data['price_change'],  # price_change
            lithium_data['price_change_percent'],  # price_change_percent
            0,  # volume (not applicable for lithium, set to 0)
            lithium_data['price_change'] < 0,  # is_decrease
            lithium_data['source'],  # source
            lithium_data['scraped_at'],  # scraped_at
            datetime.now(),  # created_at
            datetime.now()   # updated_at
        )

        # Execute the SQL query with the data
        cursor.execute(insert_query, lithium_price_data)
        
        # Commit the transaction
        connection.commit()
        print(f"Lithium price data inserted successfully: {lithium_data['price']} {lithium_data['unit']}")

    except Exception as e:
        # Rollback the transaction in case of an error
        connection.rollback()
        print(f"Error inserting lithium price data: {e}")
        raise

def _clean_numeric(value):
    """Strip currency symbols and commas, return float or 0.0"""
    try:
...
def insert_insider_transaction(cursor, connection, transaction_data):
    ...
    try:
        # Clean numeric fields — strip $, commas, etc.
        price = _clean_numeric(transaction_data.get('price', 0))
        qty = _clean_numeric(transaction_data.get('qty', 0))
        owned = _clean_numeric(transaction_data.get('owned', 0))
        value = _clean_numeric(transaction_data.get('value', 0))
